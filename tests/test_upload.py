...
def test_nfs_upload_and_verify(api, pvc_container):
    ...
    with open("/tmp/kcs-test-upload.txt", "rb") as fh:
        r = requests.post(
            f"{api}/containers/{_PVC}/upload?path=/data/hello.txt", files={"file": fh}
        )
    assert r.status_code == 200
    assert r.json()["method"] == "nfs"

    r = requests.post(
        f"{api}/containers/{_PVC}/exec", json={"command": ["cat", "/data/hello.txt"]}
    )
    assert r.status_code == 200
...
def test_upload_kubectl_cp(api, nopvc_container):
    with open("/tmp/kcs-test-upload-nopvc.txt", "rb") as fh:
        r = requests.post(
            f"{api}/containers/{_NOPVC}/upload?path=/tmp/hello.txt", files={"file": fh}
        )
